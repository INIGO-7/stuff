...
public class ProcesoCSVMeet {
	...
	// PARTE DE L脫GICA DE CSV Y DATOS
				...
				public void run() {
					...
					if (nomFic!=null && !nomFic.isEmpty()) {
						...
						if (new File(nomFic).exists()) {
							int resp = JOptionPane.showConfirmDialog( ventana, "El fichero indicado ya existe. 驴Sobreescribirlo? ", "Atenci贸n", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE );
	...
	/** Devuelve la duraci贸n en formato de horas - minutos
	 * @param minutos	Minutos de duraci贸n
	 * @return	Esa duraci贸n en formato <num> h <num> min
	 */
	 ...
	 * @param duracion	Duraci贸n en formato <num> h <num> min
	 * @return	Minutos correspondientes a esa duraci贸n
	 */
	...
	protected static void cargaCSVMeet( String fic ) {
		...
		try {
			...
			try {
				...
				Matcher m = patFecha.matcher( fic );									//Matcher interpreta patrones, y hacemos patFecha.matcher(fic) porque analizamos con el patr髇 patFecha el fichero fic.
				if (m.find()) {
					int fin = m.end();
